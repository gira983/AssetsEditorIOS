...
struct SerializedFileParser {
    func open(url: URL) throws -> SerializedFileSession {
        ...
        catch { throw SerializedFileError.malformed("could not read file") }
        ...
        let objects = metadata.objects.map { record in
            ...
            let result = header.dataOffset.addingReportingOverflow(record.byteOffset)
            let absoluteOffset = result.overflow ? header.dataOffset : result.partialValue
            return SerializedObjectInfo(id: "\(record.pathID)", pathID: record.pathID, typeID: record.typeID, byteOffset: UInt64(max(0, absoluteOffset)), byteSize: record.byteSize, typeName: typeName, displayName: "\(typeName) · \(record.pathID)")
        }
    ...
    func fields(for object: SerializedObjectInfo, in session: SerializedFileSession) throws -> [SerializedObjectField] {
        ...
        guard objectSize >= 0, start >= 0, start <= session.data.count, objectSize <= session.data.count - start else { throw SerializedFileError.malformed("object range exceeds file") }
...
struct SerializedFileSession { let data: Data; let info: SerializedFileInfo; let objects: [SerializedObjectInfo]; let objectRecords: [ObjectRecord]; let typeRecords: [SerializedTypeRecord]; let typeTreeEnabled: Bool; let header: SerializedHeader }
struct SerializedHeader { let metadataSize: UInt64; let fileSize: Int64; let version: UInt32; let dataOffset: Int64; let metadataStart: UInt64; let bigEndian: Bool }
private struct SerializedMetadata { let unityVersion: String; let targetPlatform: UInt32; let typeIDsByIndex: [Int32]; let typeCount: Int; let objects: [ObjectRecord]; let externalCount: Int; let typeTreeEnabled: Bool; let typeRecords: [SerializedTypeRecord] }
struct ObjectRecord { let pathID: Int64; let byteOffset: Int64; let byteSize: UInt32; let typeIndex: Int; let typeID: Int32; let scriptTypeIndex: UInt16 }
struct SerializedTypeRecord { let typeID: Int32; let scriptTypeIndex: UInt16; let typeTree: TypeTreeNodeRecord? }
...
private struct SerializedReader {
    ...
    mutating func readHeader() throws -> SerializedHeader {
        ...
        if version >= 22 {
            ...
            return SerializedHeader(metadataSize: largeMetadataSize, fileSize: largeFileSize, version: version, dataOffset: largeDataOffset, metadataStart: UInt64(offset), bigEndian: bigEndian)
        }
        bigEndian = endianFlag != 0
        return SerializedHeader(metadataSize: metadataSize, fileSize: fileSize, version: version, dataOffset: dataOffset, metadataStart: UInt64(offset), bigEndian: bigEndian)
    }
    ...
    mutating func readMetadata(version: UInt32) throws -> SerializedMetadata {
        ...
        var typeIDs: [Int32] = []; var typeRecords: [SerializedTypeRecord] = []
        typeIDs.reserveCapacity(typeCount); typeRecords.reserveCapacity(typeCount)
        for _ in 0..<typeCount {
            ...
            try skip(16)
            let tree: TypeTreeNodeRecord?
            if typeTreeEnabled {
                if version >= 23 {
                    try skip(16)
                    let size = try readInt32()
                    guard size >= 0 else { throw SerializedFileError.malformed("invalid type tree size") }
                    tree = size == 0 ? nil : try readTypeTree(version: version)
                } else { tree = try readTypeTree(version: version) }
                if version >= 21 { let count = try readCount(maximum: 1_000_000, label: "type dependency"); try skip(Int64(count) * 4) }
            } else { tree = nil }
            typeIDs.append(typeID); typeRecords.append(SerializedTypeRecord(typeID: typeID, scriptTypeIndex: scriptTypeIndex, typeTree: tree))
        }
        let objectCount = try readCount(maximum: 10_000_000, label: "object")
        try align(4)
        var objects: [ObjectRecord] = []; objects.reserveCapacity(objectCount)
        for _ in 0..<objectCount {
            ...
            let typeID = version >= 16 && typeIndexOrID >= 0 && Int(typeIndexOrID) < typeIDs.count ? typeIDs[Int(typeIndexOrID)] : typeIndexOrID
        ...
        if version >= 20 { let count = try readCount(maximum: 1_000_000, label: "reference type"); for _ in 0..<count { try skipReferenceTypeRecord(version: version, typeTreeEnabled: typeTreeEnabled) } }
        if version >= 5 { _ = try readCString() }
        guard offset >= metadataStart, offset <= data.count else { throw SerializedFileError.malformed("metadata exceeds file") }
        return SerializedMetadata(unityVersion: unityVersion, targetPlatform: targetPlatform, typeIDsByIndex: typeIDs, typeCount: typeCount, objects: objects, externalCount: externalCount, typeTreeEnabled: typeTreeEnabled, typeRecords: typeRecords)
    }
    ...
    mutating func skipReferenceTypeRecord(version: UInt32, typeTreeEnabled: Bool) throws {
        let typeID = try readInt32(); if version >= 16 { _ = try readUInt8() }; let script: UInt16 = version >= 17 ? try readUInt16() : UInt16.max
        if (version < 17 && typeID < 0) || (version >= 17 && typeID == 114) || (version >= 17 && script != UInt16.max) { try skip(16) }
        try skip(16)
        if typeTreeEnabled { if version >= 23 { try skip(16); let size = try readInt32(); guard size >= 0 else { throw SerializedFileError.malformed("invalid reference type tree size") }; if size > 0 { _ = try readTypeTree(version: version) } } else { _ = try readTypeTree(version: version) }; if version >= 21 { _ = try readCString(); _ = try readCString(); _ = try readCString() } }
    }
    mutating func readTypeTree(version: UInt32) throws -> TypeTreeNodeRecord? {
        if version >= 23 { guard try readUInt32() == 0x7474686d else { throw SerializedFileError.malformed("invalid extended type tree header") }; guard try readUInt32() == version else { throw SerializedFileError.malformed("type tree version mismatch") } }
        let nodeCount = try readCount(maximum: 1_000_000, label: "type tree node"); let stringLength = try readCount(maximum: 64 * 1024 * 1024, label: "type tree string table")
        var flat: [TypeTreeNodeRecord] = []; var typeOffsets: [UInt32] = []; var nameOffsets: [UInt32] = []
        for _ in 0..<nodeCount { _ = try readUInt16(); let level = Int(try readUInt8()); let flags = try readUInt8(); let typeOffset = try readUInt32(); let nameOffset = try readUInt32(); _ = try readInt32(); _ = try readUInt32(); let meta = try readUInt32(); if version >= 18 { _ = try readUInt64() }; flat.append(TypeTreeNodeRecord(type: "", name: "", level: level, flags: flags, metaFlags: meta)); typeOffsets.append(typeOffset); nameOffsets.append(nameOffset) }
        let strings = try readBytes(count: stringLength)
        let resolved = flat.enumerated().map { i, node in TypeTreeNodeRecord(type: resolve(offset: typeOffsets[i], local: strings), name: resolve(offset: nameOffsets[i], local: strings), level: node.level, flags: node.flags, metaFlags: node.metaFlags) }
        return TypeTreeNodeRecord.tree(from: resolved)
    }
    private func resolve(offset: UInt32, local: Data) -> String { let table = offset & 0x80000000 != 0 ? TypeTreeNodeRecord.commonStringTable : local; let index = Int(offset & 0x7fffffff); guard index < table.count else { return "?" }; let end = table[index...].firstIndex(of: 0) ?? table.endIndex; return String(decoding: table[index..<end], as: UTF8.self) }
    mutating func readBytes(count: Int) throws -> Data { guard count >= 0, offset <= data.count, count <= data.count - offset else { throw SerializedFileError.malformed("invalid byte range") }; defer { offset += count }; return Data(data[offset..<(offset + count)]) }
    ...
    mutating func readUInt16() throws -> UInt16 { let b = try [readUInt8(), readUInt8()]; return bigEndian ? UInt16(b[0]) << 8 | UInt16(b[1]) : UInt16(b[1]) << 8 | UInt16(b[0]) }
    mutating func readUInt32() throws -> UInt32 { let b = try [readUInt8(), readUInt8(), readUInt8(), readUInt8()]; return bigEndian ? b.reduce(0) { ($0 << 8) | UInt32($1) } : b.reversed().reduce(0) { ($0 << 8) | UInt32($1) } }
    mutating func readInt32() throws -> Int32 { Int32(bitPattern: try readUInt32()) }
    mutating func readUInt64() throws -> UInt64 { let b = try (0..<8).map { _ in try readUInt8() }; return bigEndian ? b.reduce(0) { ($0 << 8) | UInt64($1) } : b.enumerated().reduce(0) { $0 | UInt64($1.element) << (UInt64($1.offset) * 8) } }
    ...
    mutating func skip(_ count: Int64) throws { guard count >= 0, count <= Int64(data.count - offset) else { throw SerializedFileError.malformed("invalid skip") }; offset += Int(count) }
    mutating func align(_ boundary: Int) throws { let r = offset % boundary; if r != 0 { try skip(Int64(boundary - r)) } }
}

private func unityTypeName(_ id: Int32) -> String { switch id { case 1: return "GameObject"; case 28: return "Texture2D"; case 43: return "Mesh"; case 48: return "Shader"; case 49: return "TextAsset"; case 83: return "AudioClip"; case 114: return "MonoBehaviour"; case 115: return "MonoScript"; case 128: return "Font"; case 213: return "Sprite"; case 142: return "AssetBundle"; default: return "Type \(id)" } }
